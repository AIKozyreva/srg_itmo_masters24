Есть две Serratia rbidaea, надо чекнуть насколько далеко штамм из клинических образцов далек от референсного, учитывая, что эта бактерия вроде фактультативный анаэроб и вообще живёт везде. Полногеномные сравнения и визуализацию можно сделать маммером (есесена), правда ещё у него есть целый пайплайн для детекции снп и зачем мы руками это делалаи в прошлой дз не совсем понятно, ну допустим. Потом делаем другую визуализзацию через MCscan, котороый вписан в питоновскую большую либу jcvi, которая включает в себя все визуализации (на самом деле и маммер тоже) и преобразования форматов и кучу вариантов обрабатываемых форматов, и много разной статистики.

### Файлы
Ref serratia rubidaea NCBI GCF_016026735.1: fasta+fa+cds+gff 
...
Начнём с того, на что я потратила много часов в прошлой домашке, а тут это делается одной строкой за 0 секунд 🤡🤡🤡

Преобразуем `.gff` в `.bed` так, чтобы 1 CDS -> 1 название. Если где-то были дубликаты, то эта команда их сознательно проигнорирует. Ну, это наверн не очень хорошо, я потом проверю руками, но пока что я учусь, поэтому и так сойдёт.
```
...
Чтобы сработало, вы должны находится в папке, где у вас: file1.cds file1.bed, file2.cds file2.bed, причём для каждой группы часть до расширения должна называться одинаково у обоих файлов cds и bed, иначе 0. + тут нюансы с форматированием входных cds и bed файлов.
```
...
Где-то в этот момент у нас получился ещё график mummerplot-like, который как бы демонстрирует взаимное отношении референса и таргета. Должен бы указывать на перестройки. Ну rearrangements тут  точно есть кстати примерно сикось-накось, как раз как будет на итоговом графике. 
...
Далее чёта ещё непонятное, чтобы получить другие файлы `.simple`, на который у нас ссылка в нашем `layout`, при этом документации написано, что симпл - это просто типа упрощённый вариант `.anchor`(???). А потом уже, командой `karyotype` строим тот график, который нам нужен для отображения крупных перестроек. 
```
...
Но это полпути (беды), там дальше ещё миллион непонятных форматов, чтобы получить график, на котором показана идентичность участков, причё эт в моём случае, насколько я понимаю логику программы - идентичность каких-то групп гомологичных CDS. 

```
python -m jcvi.compara.synteny mcscan Strain2_serratia.bed Ref_Serratia_rubidaea.Strain2_serratia.lifted.anchors --iter=1 -o Ref_Serratia_rubidaea.Strain2_serratia.i1.blocks
python -m jcvi.formats.bed merge Strain2_serratia.bed Ref_Serratia_rubidaea.bed -o Strain2_Ref.bed
python -m jcvi.graphics.synteny Ref_Serratia_rubidaea.Strain2_serratia.i1.blocks Strain2_Ref.bed blocks.layout
```

Есть ряд вопросиков по первой команде из блока выше, потому что, если мы переставим бед файлы местами, то она почему то падает с ошибкой, что не может найти данные А в данных Б. То есть вот это `python -m jcvi.compara.synteny mcscan Ref_Serratia_rubidaea.bed Ref_Serratia_rubidaea.Strain2_serratia.lifted.anchors --iter=1 -o Ref_Serratia_rubidaea.Strain2_serratia.i1.blocks` - упало. Я не нашла похожих ошибок, поснения у меня тоже нет. Я посмотрела файлы, ему не нравилиь просто какие-то случайные области, причём когда я их удалаяла, он просто выбирал другую случайную область и такой "не могу найти". 

На картинке слева график по всем моим данным, на картинке справа я просто для понимания работы проги убрала случайный кусок строк, имеющих совпадения из файла `.i1.blocks`. Судя по всему, если у вас нормальные идентификаторы, котрые вам по сымыслу о чём-то говорят, то тогда можно убрать всё, что не является группой интереса, и сравнивать только последовательности, относящиеся к группам интереса (напрмер, только CDS с трёх конкретных генов). 

Проблема в том, что для этого надо понять, как именно устроено взаимодействие идентификаторов в каждом типе файла, потому что он требует оч странных вещей, прям СТРАННЫX, типа чтобы все названия кодирующих областей были одинаковыми, хотя понятно, что если мы берём сырые файлы, например, с ncbi, то названия хромосм (а они в гфф так или иначе в каждой фиче прописаны), будет не однотипные "chrX", типа "NC.XXXX" и "NZ.XXXX". 
